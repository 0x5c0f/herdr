...
fn mobile_tab_status(ws: &crate::workspace::Workspace) -> String {
    ...
    if ws.tabs.len() <= 1 {
        t!("tab {tab_label}", tab_label = tab_label).into_owned()
    } else {
        t!("tab {tab_label} · {current}/{total}", tab_label = tab_label, current = ws.active_tab + 1, total = ws.tabs.len()).into_owned()
    }
}
    ...
    if let Some(ws) = app.active.and_then(|idx| app.workspaces.get(idx)) {
        ...
        for (idx, tab) in ws.tabs.iter().enumerate() {
            ...
            let label: std::borrow::Cow<'_, str> = if tab.is_auto_named() {
                t!("tab {display_name}", display_name = display_name)
            } else {
                format!("{} · {display_name}", idx + 1).into()
            };
            let title = Line::from(vec![
                Span::styled("  ", Style::default().bg(bg)),
                Span::styled(
